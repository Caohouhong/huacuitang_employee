...
- (void)setModel:(EaseConversationModel *)model
{
    ...
    if ([model.title length] > 0) {
        ...
        self.titleLabel.text = model.conversation.conversationId;
    }
    
    if (self.showAvatar) {
        if ([model.avatarURLPath length] > 0){
            [self.avatarView.imageView sd_setImageWithURL:[NSURL URLWithString:model.avatarURLPath] placeholderImage:model.avatarImage];
        } else {
    ...
    if (model.conversation.unreadMessagesCount == 0) {
        ...
        _avatarView.badge = model.conversation.unreadMessagesCount;
    }
}
