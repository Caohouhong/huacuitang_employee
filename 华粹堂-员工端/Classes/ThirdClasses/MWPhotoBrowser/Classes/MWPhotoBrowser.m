...
- (void)performLayout {
    ...
    else if ([self.navigationController.viewControllers objectAtIndex:1] == self)
    {
        _doneButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"发布照片预览_删除"] style:UIBarButtonItemStylePlain target:self action:@selector(deleteImage)];
        self.navigationItem.rightBarButtonItem = _doneButton;
    }
    else
    {
...
- (void)savePhotos:(UILongPressGestureRecognizer *)longPress
{
    if (longPress.state == UIGestureRecognizerStateBegan)
    {
        ...
        LCActionSheet *actionSheet = [[LCActionSheet alloc] initWithTitle:nil buttonTitles:@[@"保存图片"] redButtonIndex:-1 delegate:nil];
        actionSheet.didClickedButtonAtIndex = ^(NSInteger btnIndex,NSString *title){
...
- (void)deleteImage
{
    if ([self.delegate respondsToSelector:@selector(deleteImageWithImageIndex:)]) {
        [self.delegate deleteImageWithImageIndex:(int)self.currentIndex];
    }
}
