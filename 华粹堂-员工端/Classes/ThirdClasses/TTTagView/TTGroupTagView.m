...
- (void)layoutTags {
    ...
    if(!_tagButtons.count) {
        self.changeHeight = 0;
    }
    self.changeHeight = _svContainer.frame.size.height;
    
    DLog(@"height:%f", self.changeHeight);
}

// 添加标签按钮
- (TTTagViewCheckBoxButton *)tagButtonWithTag:(NSString *)tag withCornerScale:(double)scale{
    ...
    tagBtn.layer.cornerRadius = btnFrame.size.height * scale;
...
#pragma mark - 添加标签
//tag 和 圆角比列
- (void)addTags:(NSArray *)tags withCornerScale:(double) scale {
    
    for (NSString *tag in tags) {
        [self addTagToLast:tag withScale:scale];
    }
    
    [self layoutTags];
}

- (void)addTagToLast:(NSString *)tag withScale:(double) scale{
    
    NSArray *result = [_tagStrings filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"SELF == %@", tag]];
    if (result.count == 0) {
        [_tagStrings addObject:tag];
        
        TTTagViewCheckBoxButton* tagButton = [self tagButtonWithTag:tag withCornerScale:scale];
