...
- (void)loadDatas
{
    ...
    [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:self.arrayMonths.count/2 inSection:0] atScrollPosition:UITableViewScrollPositionTop animated:YES];
...
- (NSArray *)createArrayMonths
{
    ...
    self.firstDate = [NSDate dateWithTimeInterval:-63072000 sinceDate:[NSDate date]];
...
#warning 待定（显示这天房源的房态）
    ...
    if (self.checkInTime && self.checkOutTime)
    {
        [self clearDates];
        [self.tableView reloadData];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if ([self.delegate respondsToSelector:@selector(selectedDate:and:)])
            {
                [self.delegate selectedDate:btn.date and:nil];
            }
    ...
    else
    {
        if(self.checkInTime != nil && [btn.date timeIntervalSinceDate:self.checkInTime]>0)
        {
            ...
            if ([self.delegate respondsToSelector:@selector(selectedDate:and:)])
            {
                [self.delegate selectedDate:self.checkInTime and:btn.date];
            }
        }
    }
}
