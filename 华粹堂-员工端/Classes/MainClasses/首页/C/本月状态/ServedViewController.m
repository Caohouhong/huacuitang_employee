...
@interface ServedViewController ()<UITableViewDelegate, UITableViewDataSource>
{
...
@implementation ServedViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = (self.viewType == ServedVCTypeHaveServed) ? @"本月已服务" : @"本月待服务";
    self.pageNo = 1;
    [self drawView];
    //show hud
...
- (void)initHeaderView{
    ...
    if (self.viewType == ServedVCTypeHaveServed){
        topTextlabel.text = [NSString stringWithFormat:@"时间:%@    已服务人次：0",[HuaCuiTangHelper getCurrentDateWithStyleYYYYMM]];
    }else {
        topTextlabel.text = [NSString stringWithFormat:@"时间:%@    待服务人次：0",[HuaCuiTangHelper getCurrentDateWithStyleYYYYMM]];
    }
...
{
    ServeTableViewCell *cell = [ServeTableViewCell cellWithTableView:self.tableView];
...
- (void)requestData
{
    ...
    [params setValue:(self.viewType == ServedVCTypeHaveServed) ? @(1) : @(0) forKey:@"state"]; //0 - 本月待服务  1-本月已服务
    [params setValue:@(self.pageNo) forKey:@"pageNo"];
    [params setValue:@(20) forKey:@"pageSize"];
    
    [HCTConnet getHomeMonthServe:params success:^(id responseObject) {
...
//刷新界面
- (void)refreshUIWith:(NSDictionary *)responseObject{
    
    NSArray *array = [responseObject objectForKey:@"data"];
    if ([array count] > 0){
        
        if (self.viewType == ServedVCTypeHaveServed){
            topTextlabel.text = [NSString stringWithFormat:@"时间:%@  已服务人次：%@",[HuaCuiTangHelper getCurrentDateWithStyleYYYYMM], [array.firstObject objectForKey:@"serviceTotal"]];
        }else {
            topTextlabel.text = [NSString stringWithFormat:@"时间:%@  待服务人次：%@",[HuaCuiTangHelper getCurrentDateWithStyleYYYYMM], [array.firstObject objectForKey:@"serviceTotal"]];
        }
